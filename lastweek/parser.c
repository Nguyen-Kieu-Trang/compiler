...
ConstantValue *compileUnsignedConstant(void) {
    ...
    switch (lookAhead->tokenType) {
        ...
        case TK_FLOAT:
            eat(TK_FLOAT);
            constValue = makeFloatConstant(currentToken->valueF);
            break;
        case TK_IDENT:
            eat(TK_IDENT);
...
void compileForSt(void) {
    // TODO: Check type consistency of FOR variable
    eat(KW_FOR);
    eat(TK_IDENT);

    // check if the identifier is a variable
    Object *object = checkDeclaredVariable(currentToken->string);

    if (object->typeAttrs->actualType->typeClass == TP_FLOAT)
        error(ERR_FLOAT_IN_LOOP, currentToken->lineNo, currentToken->colNo);
